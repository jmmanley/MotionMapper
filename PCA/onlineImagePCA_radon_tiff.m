function [mu,vecs,vals,vecsS,valsS] = onlineImagePCA_radon_tiff(path,batchSize,scale,pixels,thetas,numPerFile,file_list)
...
if isempty(pixels)
    testRadon = radon(testImage, thetas);
    pixels = 1:length(testRadon(:));
end
L = length(pixels);
